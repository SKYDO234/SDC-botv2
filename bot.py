...
def parse_size_to_bytes(size_str: str) -> int:
    ...
    # Enforce minimum memory safeguard (Minimum 256MB to avoid Docker 6MB error)
...
# ---------------------------------------------------------
# TMATE ENGINE PIPELINE (FAST NON-BLOCKING HANDSHAKE)
# ---------------------------------------------------------
def _setup_tmate_sync(container_id: str) -> str:
    """Fast, non-blocking tmate setup inside running container."""
    container = docker_client.containers.get(container_id)
    
    # Run setup script in background inside container
    init_cmd = (
        "bash -c '"
    ...
    container.exec_run(init_cmd, detach=True)

    # Poll for tmate SSH connection string (up to 30 seconds max)
    ...
    raise TimeoutError("tmate server connection failed or timed out. Check network connection.")

async def setup_tmate_container(container) -> str:
    return await asyncio.to_thread(_setup_tmate_sync, container.id)

# ---------------------------------------------------------
# OS PROVISIONING PIPELINE (OFFICIAL IMAGE REUSE)
# ---------------------------------------------------------
def _build_and_run_sync(os_type: str, ram_bytes: int, cpu_cores: int, disk_bytes: int, container_name: str):
    ...
    # Map requested OS to official Docker base images to eliminate slow builds
    image_map = {
    ...
    # Run container instantly
    container = docker_client.containers.run(
        image=image_tag,
        name=container_name,
        command="tail -f /dev/null",  # Keeps container running continuously
        detach=True,
        tty=True,
        stdin_open=True,
        mem_limit=ram_bytes,
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        # Step 1: Instant Container Spin Up
        ...
        # Step 2: Configure tmate in background
        ...
        # Step 3: Save DB & Send Direct Message
        db = load_db()
